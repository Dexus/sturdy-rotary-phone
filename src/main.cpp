...
void setupWiFi()
{
  ...
  String body = String("sourdough_alcohol_limit value=1\n") +
                "sourdough_heigh value=2";
  // Submit POST request via HTTP
  httpInflux.begin(String("https://nc.josef-froehle.de/"), ROOT_CA);
  httpInflux.addHeader("Authorization", INFLUX_TOKEN);

  int httpCode2 = httpInflux.POST(body);
  if (httpCode2 > 0)
  {
    Serial.printf("Influx [HTTPS] POST...  Code: %d  Response: ", httpCode2);
    Serial.println(httpInflux.getLocation());
    Serial.println();
  }
  else
  {
    Serial.printf("Influx [HTTPS] POST... Error: %s\n", httpInflux.errorToString(httpCode2).c_str());
  }
  ...
  // Submit POST request via HTTP
  httpInflux.begin(*client, String("https://nc.josef-froehle.de/"));
  httpInflux.addHeader("Authorization", INFLUX_TOKEN);

  int httpCode = httpInflux.GET();
  if (httpCode > 0)
  {
    Serial.printf("Influx [HTTPS] GET...  Code: %d  Response: ", httpCode);
    Serial.println();
  }
  else
  {
    Serial.printf("Influx [HTTPS] GET... Error: %s\n", httpInflux.errorToString(httpCode).c_str());
  }
...
void submitHostedInflux(unsigned long ts, float cels, float hum, float hic, long height, int alcohol_value, int alcohol_limit)
{
  ...
  // Submit POST request via HTTP
  httpInflux.begin(*client, String("https://") + INFLUX_HOST + "/api/v2/write?org=" + INFLUX_ORG_ID + "&bucket=" + INFLUX_BUCKET + "&precision=s");
  httpInflux.addHeader("Authorization", INFLUX_TOKEN);
